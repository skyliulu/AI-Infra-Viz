// LLMInference.jsx
// ─────────────────────────────────────────────────────────────────
// Paste your LLM Inference visualization logic here.
...
const App = () => {
  ...
  const displayProbs = React.useMemo(() => {
            ...
            <div className="bg-white rounded-2xl p-6 shadow-sm border border-slate-200">
              ...
              <div className="flex flex-wrap gap-1 mt-4">
                {Array.from({ length: MOCK_PROMPT_TOKENS.length + MOCK_GENERATED_TOKENS.length - 1 }).map((_, i) => (
                  <div key={i} className={`h-8 flex-1 rounded-sm transition-all duration-300 ${i < kvCacheSize ? (i < MOCK_PROMPT_TOKENS.length ? 'bg-blue-400' : 'bg-green-400') : 'bg-slate-100'}`} title={i < kvCacheSize ? `Cached Token ${i}` : 'Empty Space'}></div>
                ))}
              </div>
            </div>

            <div className="bg-indigo-900 text-indigo-50 rounded-2xl p-6 shadow-lg">
              <h3 className="text-lg font-bold mb-4 text-white">当前微观执行状态</h3>
              ...
              <div className="space-y-4 text-sm leading-relaxed">
                ...
                {activeModule === 6 && (
                  <div className="animate-fade-in text-center py-4 border-t border-indigo-700 mt-4 pt-4">
                    <div className="inline-block p-2 bg-emerald-800 rounded-full mb-2"><Zap className="text-emerald-400" size={20} /></div>
                    <h4 className="font-bold text-emerald-300 text-base">Token 生成完毕</h4>
                    <p className="opacity-80 mt-1 text-xs">通过采样掷骰子选中下一个词，准备丢回起点循环。</p>
                  </div>
                )}
              </div>
            </div>
